...
| 文件 | 内容 | 说明 |
|------|------|------|
| `kg/entities/data/person_lifespans.json` (v1) | 156 人，点值格式 `{"birth": -551, "death": -479}` | 来自外部百科，作为 v2 的外部参考源 |
| `kg/entities/data/person_lifespans_v2.json` | **512 人**，区间格式 | 由 `infer_lifespans.py` 合并 reign_periods + 文本模式 + 事件索引 + v1 派生 |
| `docs/special/lifespan.html` | — | v2 数据的渲染页（置信度分级 + 证据链 + 搜索/筛选）|
...
#  输出样例：512 人 {'medium': 465, 'low': 38, 'legend': 6, 'high': 3}
#  证据来源：reign_periods ×370 | lifespans_v1 ×151 | event_index ×73 | text_pattern ×20

# Step 2: 渲染专项页
python kg/entities/scripts/render_lifespan_page.py
#  → docs/special/lifespan.html（按时代分组，证据列可见）
```
...
**输出文件**：`kg/entities/data/person_lifespans_v2.json`（已落地，2026-04-22 首版）

**v1 首版实现落地情况**（2026-04-22）：

| 来源 | 条数 | 置信度 |
|------|------|--------|
| reign_periods 全量 | 370 rulers | medium（生年区间宽 ±25 年） |
| 文本模式 AGE_AT_ACC | 2（秦宁公、秦出子） | high（生年点值） |
| 文本模式 AGE_AT_DEATH | 1（张苍） | high（生年反推自 100 岁卒） |
| 文本模式 REIGN_LEN | 31（立 X 年卒） | 与 reign_periods 做一致性校验 |
| 事件索引卒年 | 73 非君主人物（子产、伍子胥、萧何…） | medium（卒年点值，生年未知） |
| v1 lifespans 外部 | 151（别名合并后） | medium / legend |
| **合并总计** | **512 人** | — |

**尚未覆盖的场景**：
- 方法 4：亲属关系约束（父卒 < 子生、兄弟生年差 < 40）— 需要 `kg/genealogy/` 关系数据
- 方法 5：世代数推算（"X 世孙"）— 低可靠度，暂不实现
- 传说时代的宽区间（尧舜禹等）已用 ±30 年标记，但仍可能过窄
